...
public class ConsolePegPrinter implements PegPrinter {
    ...
    public ConsolePegPrinter(Peg peg1, Peg peg2, Peg peg3) {
        ...
        display = new char[high][width]; // картинка со стержнями и дисками
    ...
    /**
     * Отобразить стержни с дисками в консоли
     */
    public void draw() {
        initArray();
        drawPegs();
        ...
        if (!peg.getDisks().isEmpty()) {
            ...
            for (int i = 0; i<size; i++) {
                for (int j = 0; j<=disks.get(i).r(); j++) {
                    ...
                    display[high-1-i][pegPosition+j] = PEG_SYMBOL; // и справа от стрежня
                }
            }
        }
    }
